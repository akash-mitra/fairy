...
# create a test file
echo "This is a static file for testing" > ${WEBROOT}/${SITENAME}/public_html/index.html
# change ownership
...
echo "        root ${WEBROOT}/${SITENAME}/public_html;"                        >> /etc/nginx/sites-available/${SITENAME}
...
echo "                try_files \$uri \$uri/ =404;"                            >> /etc/nginx/sites-available/${SITENAME}
...
gather "Root directory of ${SITENAME} is located at ${WEBROOT}/${SITENAME}/public_html"
...
echo "<HTML><HEAD><TITLE>Fairy $VERSION - PHP Test Page</TITLE></HEAD>"        > ${WEBROOT}/${SITENAME}/public_html/info.php
echo "<BODY><CENTER><H1>Test Page</H1><H6>Fairy Version $VERSION</H6><HR />"   >> ${WEBROOT}/${SITENAME}/public_html/info.php
echo "<?php echo 'Current script owner: ' . get_current_user() . '<br />' . 'Server: ' . \$_SERVER['SERVER_ADDR']; ?> " >> ${WEBROOT}/${SITENAME}/public_html/info.php
echo "</CENTER></BODY></HTML>"                                                 >> ${WEBROOT}/${SITENAME}/public_html/info.php
echo "<?php phpinfo(); ?>"                                                     >> ${WEBROOT}/${SITENAME}/public_html/info_.php
# change ownership
chown ${SITEUSER}:${SITEUSER} ${WEBROOT}/${SITENAME}/public_html/*
...
echo "${CHANGE_STAMP}"  >> /etc/php/7.0/mods-available/apcu.ini
echo "apc.shm_size = ${APC_CACHE_MEM_SIZE}" >> /etc/php/7.0/mods-available/apcu.ini
update_change_log "/etc/php/7.0/mods-available/apcu.ini" "setting APC's memory size to ${APC_CACHE_MEM_SIZE}"
# apc.enabled=1
# apc.shm_segments=1
# apc.shm_size=64M
# apc.ttl=7200
...
echo "    server unix:/var/run/php/php7.0-fpm.sock;" >> /etc/nginx/conf.d/php-sock.conf
